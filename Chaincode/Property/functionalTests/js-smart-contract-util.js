...
class SmartContractUtil {
    ...
    static async getConnectionProfile() {
        ...
        const connectionProfilePath = path.join(homedir, '.fabric-vscode', 'environments', 'PropertyNetwork', 'gateways', 'User Gateway.json');
    ...
    static async submitTransaction(contractName, functionName, args, gateway) {
        ...
        let contract;
        if (contractName !== '') {
            contract = await network.getContract('Property', contractName);
        } else {
            contract = await network.getContract('Property');
        }
